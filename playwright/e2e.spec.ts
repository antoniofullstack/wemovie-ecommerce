...
test.describe('WeMovies E2E Purchase Flow', () => {
  ...
  test('should complete a full purchase journey successfully', async ({ page }) => {
    // 1. Verify Home Page loads with movies
    await expect(page.locator('h2').first()).toBeVisible();
    
    // 2. Add "Viúva Negra" to cart
    const viuvaCard = page.locator('[data-testid^="movie-card-"]', { hasText: 'Viúva Negra' });
    ...
    const shangCard = page.locator('[data-testid^="movie-card-"]', { hasText: 'Shang-Chi' });
    ...
    const viuvaRow = page.locator('[data-testid^="cart-item-"]', { hasText: 'Viúva Negra' });
    ...
    const shangRow = page.locator('[data-testid^="cart-item-"]', { hasText: 'Shang-Chi' });
    ...
    await expect(page.getByText('Total').locator('xpath=following-sibling::span')).toContainText('R$ 59,98');
  ...
  test('should handle empty cart state and deep linking', async ({ page }) => {
    ...
    await expect(page.getByText('Parece que não há nada por aqui :(')).toBeVisible();
    
    // 3. Click back button (Reload page in this app context often points back or reloads)
    const backButton = page.getByRole('button', { name: /voltar/i }).or(page.getByRole('button', { name: /recarregar página/i }));
