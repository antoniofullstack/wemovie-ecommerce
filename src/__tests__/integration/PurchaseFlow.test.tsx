...
describe("Integration Flow: Purchase Journey", () => {
  ...
  it("should allow a user to add movies to cart and complete the purchase", async () => {
    ...
    const viuvaCard = screen.getByText("Viúva Negra").closest("div")?.parentElement;
    const addViuvaButton = within(viuvaCard!).getByRole("button", { name: /adicionar ao carrinho/i });
    ...
    const shangCard = screen.getByText("Shang-Chi").closest("div")?.parentElement;
    const addShangButton = within(shangCard!).getByRole("button", { name: /adicionar ao carrinho/i });
    ...
    const viuvaRow = screen.getByText("Viúva Negra").closest("div")?.parentElement;
